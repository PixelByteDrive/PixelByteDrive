exports.run = async(Discord, client, message, args) => {
  if(!args[0]) return message.reply("Yo, don't you know that you need to type more arguements?");
   ...
   .setColor("BLUE")
   .setThumbnail(sicon)
   .addField("This feedback was by", message.author)
   .addField("Feedback", suggestion)
   .addField("It was sent in", message.guild.name)
   .setTimestamp()
   
   let msg = await client.channels.get(`584813322373824522`).send(suggestionEmbed).then(function(msg) {
     message.delete({timeout: 1000});
   }).catch(function(error) {
     console.log(error);
   });
   
   message.channel.send("Your feedback was sent, thank you!")
}